...
ParameterType = {0: "Static", 1: "Dynamic", 2: "Dynamic32"}
Comparison = {0: "EqualTo", 1: "GreaterThan", 2: "LessThan", 3: "OneOf"}
...
# add_txn_with_role
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def add_txn_with_role(
    roles_mod_address, to_address, tx_data, eth_value, json_file, web3=None
):
    ...
    exec_data = get_data(
        ...
        [to_address, int(eth_value), tx_data, 0, 1, False],
        ETHEREUM,
        web3=web3,
        abi_address=ROLES_MOD_PROXY,
    )
    if exec_data is not None:
        json_file["transactions"].append(
            {"to": roles_mod_address, "data": exec_data, "value": str(int(0))}
        )
...
def approve_token(
    ...
    if (token == WETH_ETH and eth == False) or token != WETH_ETH:
        ...
        if (
            token0_contract.functions.allowance(avatar_address, spender_address).call()
            == 0
        ):
            tx_data = get_data(
                token,
                "approve",
                [spender_address, MAX_TOKEN_AMOUNT],
                ETHEREUM,
                abi_address=TOKEN_PROXY,
                web3=web3,
            )
            if tx_data is not None:
                add_txn_with_role(
                    roles_mod_address, token, tx_data, 0, json_file, web3=web3
                )
...
def json_file_download(json_file):
    ...
    file_path = (
        str(Path(os.path.abspath(__file__)).resolve().parents[1])
        + "/%s.json" % file_name
    )
    print()
    try:
        ...
        message = "JSON file %s was succesfully downloaded to the path: %s" % (
            "%s.json" % file_name,
            file_path,
        )
...
def continue_execution(json_has_txns=False):
    ...
    if not json_has_txns:
        print(
            f"{bcolors.WARNING}{bcolors.BOLD}No transactions were recorded{bcolors.ENDC}"
        )
        print()
...
def get_contract_name_from_scan(contract_address, blockchain):
    ...
    if blockchain == ETHEREUM:
        data = requests.get(
            API_ETHERSCAN_GETSOURCECODE % (contract_address, API_KEY_ETHERSCAN)
        ).json()

    elif blockchain == POLYGON:
        data = requests.get(
            API_POLYGONSCAN_GETSOURCECODE % (contract_address, API_KEY_POLSCAN)
        ).json()

    elif blockchain == XDAI:
        data = requests.get(
            API_GNOSISSCAN_GETSOURCECODE % (contract_address, API_KEY_GNOSISSCAN)
        ).json()

    elif blockchain == BINANCE:
        data = requests.get(
            API_BINANCE_GETSOURCECODE % (contract_address, API_KEY_BINANCE)
        ).json()

    elif blockchain == AVALANCHE:
        data = requests.get(
            API_AVALANCHE_GETSOURCECODE % (contract_address, API_KEY_AVALANCHE)
        ).json()

    elif blockchain == FANTOM:
        data = requests.get(
            API_FANTOM_GETSOURCECODE % (contract_address, API_KEY_FANTOM)
        ).json()

    elif blockchain == OPTIMISM:
        data = requests.get(
            API_OPTIMISM_GETSOURCECODE % (contract_address, API_KEY_OPTIMISM)
        ).json()

    elif blockchain == ARBITRUM:
        data = requests.get(
            API_ARBITRUM_GETSOURCECODE % (contract_address, API_KEY_ARBITRUM)
        ).json()
...
def decode_data(contract_address, data, blockchain):
    ...
    proxy_impl_address = search_proxy_impl_address(
        contract_address, blockchain, web3=web3
    )

    if proxy_impl_address != ZERO_ADDRESS:
        contract = get_contract_proxy_abi(
            contract_address, proxy_impl_address, blockchain, web3=web3
        )
    else:
    ...
    func_sig = requests.get(
        "https://api.openchain.xyz/signature-database/v1/lookup?function=%s&filter=true"
        % data[:10]
    ).json()["result"]["function"][data[:10]][0]["name"]
    ...
    for func_param in func_params:
        if isinstance(func_params[func_param], bytes):
            ...
            if func_param == "functionSig":
                ...
                selector_names = requests.get(
                    "https://api.openchain.xyz/signature-database/v1/lookup?function=%s&filter=true"
                    % selector
                ).json()["result"]["function"][selector]

                # If the contract does not have the function, it checks if there is a proxy implementation
                proxy_impl_address = search_proxy_impl_address(
                    func_params["targetAddress"], blockchain, web3=web3
                )

                if proxy_impl_address != ZERO_ADDRESS:
                    target_contract = get_contract_proxy_abi(
                        func_params["targetAddress"],
                        proxy_impl_address,
                        blockchain,
                        web3=web3,
                    )
                else:
                    target_contract = get_contract(
                        func_params["targetAddress"], blockchain
                    )
                ...
                for selector_name in selector_names:
                    try:
                        getattr(
                            target_contract.functions,
                            selector_name["name"][: selector_name["name"].index("(")],
                        )
                ...
                if signature != "":
                    ...
                    matching_functions = get_abi_function_signatures(
                        func_params["targetAddress"],
                        blockchain,
                        func_names=[signature[: signature.index("(")]],
                    )
                    for matching_function in matching_functions:
                        if matching_function["signature"] == signature:
                            break
                    description = get_function_description(
                        signature, matching_function["components_names"]
                    )
                else:
        ...
        elif isinstance(func_params[func_param], tuple) or isinstance(
            ...
            for i in range(len(func_params[func_param])):
                if func_param == "paramType":
                    func_params[func_param][i] = (
                        str(func_params[func_param][i])
                        + ": "
                        + ParameterType[func_params[func_param][i]]
                    )
                elif func_param == "paramComp":
                    func_params[func_param][i] = (
                        str(func_params[func_param][i])
                        + ": "
                        + Comparison[func_params[func_param][i]]
                    )
            ...
            if func_param == "paramType":
                func_params[func_param] = (
                    str(func_params[func_param])
                    + ": "
                    + ParameterType[func_params[func_param]]
                )
