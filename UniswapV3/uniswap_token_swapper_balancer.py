...
TOKENS_OUT = [SETH2_ETH, USDC_ETH, DAI_ETH, USDT_ETH, WETH_ETH, WBTC_ETH]
...
PATHS = {
    ...
    SETH2_ETH: {WETH_ETH: [SETH2_ETH, WETH_ETH]},
    WETH_ETH: {
    ...
    WBTC_ETH: {WETH_ETH: [WBTC_ETH, WETH_ETH]},
}
...
while True:
    ...
    print(
        f"{bcolors.OKBLUE}{bcolors.BOLD}Select where you want to execute the token swappings:{bcolors.ENDC}"
    )
    ...
    while token_option not in valid_token_options:
        message = (
            ...
            + ",".join(option for option in valid_token_options)
            + "): "
        )
    ...
    if int(token_option) <= i - 2:
        ...
        if token_balance > 0:
            ...
            message = (
                str(
                    "The amount of %s in the Avatar Safe is: %.18f"
                    % (token_symbol, token_balance / (10**selected_token_decimals))
                )
                .rstrip("0")
                .rstrip(".")
            )
            ...
            print()
            message = (
                'If you want to select the MAX amount of %s enter "max"' % token_symbol
            )
                        ...
                        if amount > token_balance:
                            ...
                            message = (
                                str(
                                    ...
                                    % (
                                        token_symbol,
                                        token_balance / (10**selected_token_decimals),
                                    )
                                )
                                .rstrip("0")
                                .rstrip(".")
                            ...
                            message += (" %s\n") % token_symbol
                            print(
                                f"{bcolors.FAIL}{bcolors.BOLD}{message}{bcolors.ENDC}"
                            )
                            raise Exception
                    break
                except:
            ...
            if len(SWAP_TOKENS) == 1:
                ...
                for swap_token in SWAP_TOKENS:
                    print(
                        "%d- %s" % (j + 1, get_symbol(swap_token, ETHEREUM, web3=web3))
                    )
                ...
                while swap_token_option not in valid_swap_token_options:
                    message = (
                        "Enter a valid option ("
                        + ",".join(option for option in valid_swap_token_options)
                        + "): "
                    )
            ...
            selected_swap_token_symbol = get_symbol(
            ...
            if uniswap_option == "1":
                ...
                path = select_path(
                    PATHS[selected_token][selected_swap_token], amount, web3=web3
                )
            else:
                # fee = select_fee(token_symbol, selected_swap_token_symbol, web3=web3)
                amount_out_min, fee = set_min_amount_out_and_fee(
                    selected_token,
                    selected_swap_token,
                    amount,
        ...
        if balance > 0:
            ...
            if selected_token == ZERO_ADDRESS:
                ...
                tx_data = get_data(
                    WETH_ETH, "withdraw", [int(amount)], ETHEREUM, web3=web3
                )
