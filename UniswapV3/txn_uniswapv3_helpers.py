...
def select_path(paths, web3=None):
    ...
    if isinstance(paths[0], list):
        ...
        for path in paths:
            path_symbols = [
                get_symbol(path_token, ETHEREUM, web3=web3) for path_token in path
            ]
        ...
        while int(path_option) not in valid_path_options:
            message = (
                ...
                + ",".join(str(option) for option in valid_path_options)
                + "): "
            )
        ...
        path_symbols = [
            get_symbol(path_token, ETHEREUM, web3=web3) for path_token in paths
        ]
        path_string = "[{}]".format("->".join(path_symbols))
        path = paths
...
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def set_min_amount_out_and_fee(
    selected_token, selected_swap_token, amount, selected_swap_token_symbol, web3=None
):
    ...
    quoter_contract = get_contract(
        UNISWAPV3_QUOTER, ETHEREUM, web3=web3, abi=ABI_QUOTER_V3
    )
        ...
        try:
            amounts_out.append(
                quoter_contract.functions.quoteExactInputSingle(
                    selected_token, selected_swap_token, int(fee), int(amount), int(0)
                ).call()
            )
        except:
    ...
    selected_swap_token_decimals = get_decimals(
        selected_swap_token, ETHEREUM, web3=web3
    )

    print(f"{bcolors.OKBLUE}{bcolors.BOLD}Best quote results:{bcolors.ENDC}")
    message = (
        str("Amount Out: %.18f" % (amount_out / 10**selected_swap_token_decimals))
        .rstrip("0")
        .rstrip(".")
    )
    message += "\nFee: %s%%\n" % (str(fee / 10000))
    ...
    message = (
        'If you want to select the MAX Amount Out of %s enter "max"'
        % selected_swap_token_symbol
    )
    print(f"{bcolors.WARNING}{bcolors.BOLD}{message}{bcolors.ENDC}")
    input_amount_out = input(
        "Enter the Amount Out of %s: " % selected_swap_token_symbol
    )
    while True:
        try:
            if input_amount_out == "max":
                ...
                if input_amount_out > amount_out:
                    ...
                    message = (
                        str(
                            "Input Amount Out must be lower than %.18f"
                            % (amount_out / 10**selected_swap_token_decimals)
                        )
                        .rstrip("0")
                        .rstrip(".")
                    )
...
def select_fee(selected_token, selected_swap_token, web3=None):
    ...
    print(
        "Select the Fee for the UniswapV3 %s/%s pool: "
        % (selected_token, selected_swap_token)
    )
    print()
    print("1- 0.01%")
    print("2- 0.05%")
...
def get_rate(path, web3=None):
    ...
    for i in tqdm(range(len(path) - 1)):
        ...
        for fee in tqdm(FEES):
            rate = get_rate_uniswap_v3(
                path[i], path[i + 1], "latest", ETHEREUM, web3=web3, fee=fee
            )
            if rate != None:
                rates.append(rate)
...
def swap_selected_token_v2(
    ...
    tx_data = get_data(
        ...
        [int(round(token_balance)), int(round(amount_out_min)), path, avatar_address],
        ETHEREUM,
        web3=web3,
    )
    if tx_data is not None:
        add_txn_with_role(
            roles_mod_address, UNISWAPV3_ROUTER2, tx_data, 0, json_file, web3=web3
        )
    ...
    tx_data = get_data(
        ...
        [
            [
                ...
                int(fee),
                avatar_address,
                int(round(token_balance)),
                int(round(amount_out_min)),
                0,
            ]
        ],
        ETHEREUM,
...
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def get_amount1(
    positions_nft_contract, pool_contract, nft_position_id, amount0_desired, web3=None
):
    ...
    amount1_desired = Decimal(
        ...
        * Decimal(
            sqrt_price_x96
            * 1.0001 ** (tick_upper / 2)
            * (sqrt_price_x96 - 1.0001 ** (tick_lower / 2) * (2**96))
        )
...
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def get_amount0(
    positions_nft_contract, pool_contract, nft_position_id, amount1_desired, web3=None
):
...
def get_amount1_mint(
    ...
    if pool_contract == None:
        ...
        amount1_desired = int(
            Decimal(amount0_desired)
            * sqrt_price
            * Decimal(1.0001) ** Decimal(tick_upper / 2)
            * Decimal(10 ** ((token1_decimals + token0_decimals) / 2))
            * (
                sqrt_price * Decimal(10 ** ((token1_decimals - token0_decimals) / 2))
                - Decimal(1.0001) ** Decimal(tick_lower / 2)
            )
            / (
                Decimal(1.0001) ** Decimal(tick_upper / 2)
                - sqrt_price * Decimal(10 ** ((token1_decimals - token0_decimals) / 2))
        ...
        amount1_desired = Decimal(
            ...
            * Decimal(
            ...
            (2**96) * (1.0001 ** (tick_upper / 2) * (2**96) - sqrt_price_x96)
        )
...
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def get_selected_token_balance(
    avatar_address, token_address, token_symbol, token_decimals, web3=None
):
    ...
    selected_token_balance = balance_of(
        avatar_address, token_address, "latest", ETHEREUM, decimals=False, web3=web3
    )
    ...
    if selected_token_balance == 0:
        ...
        message = (
            (
                "The balance of %s in the Avatar Safe is %.18f"
                % (token_symbol, selected_token_balance / (10**token_decimals))
            )
            .rstrip("0")
            .rstrip(".")
        )
...
def get_removed_liquidity(
    ...
    nft_position_balances = underlying(
        avatar_address, nft_position_id, "latest", ETHEREUM, web3=web3, decimals=False
    )

    if nft_position_balances == None:
        print(
            f"{bcolors.FAIL}{bcolors.BOLD}Error while retrieving the position balances{bcolors.ENDC}"
        )
        exit()
    ...
    try:
        nft_position_balances[1]
    except IndexError:
        nft_position_balances.append([token1, Decimal(0)])

    message = (
        (
            "The balance of %s in the NFT Position is %.18f"
            % (token0_symbol, nft_position_balances[0][1] / (10**token0_decimals))
        )
        .rstrip("0")
        .rstrip(".")
    )
    message += (
        (
            "\nThe balance of %s in the NFT Position is %.18f"
            % (token1_symbol, nft_position_balances[1][1] / (10**token1_decimals))
        )
        .rstrip("0")
        .rstrip(".")
    )
    ...
    print(
        f"{bcolors.WARNING}{bcolors.BOLD}The percentage must be greater than 0% and lower or equal to 100%{bcolors.ENDC}"
    )
    percentage_removed_liquidity = input(
        "Enter the percentage of liquidity to remove: "
    )
    while True:
        try:
    ...
    print()
    amount0_desired = nft_position_balances[0][1] * Decimal(
        percentage_removed_liquidity / 100
    )
    amount1_desired = nft_position_balances[1][1] * Decimal(
        percentage_removed_liquidity / 100
    )

    message = (
        (
            "The balance of %s to be removed is %.18f"
...
def get_amounts_desired(
    ...
    amount0_desired = 0
    amount1_desired = 0
    message = 'If you want to select the MAX amount of %s enter "max"' % (
        selected_token_symbol
    )
    print(f"{bcolors.WARNING}{bcolors.BOLD}{message}{bcolors.ENDC}")
        ...
        try:
            if token_option == "1":
                ...
                if amount0_desired > selected_token_balance:
                    ...
                    message = (
                        (
                            "Insufficient balance of %s in Avatar Safe\nThe balance of %s is %.18f"
                            % (
                                selected_token_symbol,
                                selected_token_symbol,
                                selected_token_balance / (10**selected_token_decimals),
                            )
                        )
                        .rstrip("0")
                        .rstrip(".")
                ...
                if operation == "1":
                    amount1_desired = get_amount1_mint(
                        pool_contract,
                        tick_lower,
                        tick_upper,
                        amount0_desired,
                ...
                if token1_balance < amount1_desired:
                    ...
                    message = (
                        (
                            "Insufficient balance of %s in Avatar Safe: %.18f"
                            % (
                                token1_symbol,
                                balance_of(
                                    avatar_address,
                    ...
                    message += (
                        (
                            " %s\nThe desired amount of %s is %.18f"
                            % (
                                token1_symbol,
                                token1_symbol,
                ...
                if operation == "1":
                    amount0_desired = get_amount0_mint(
                        pool_contract,
                        tick_lower,
                        tick_upper,
                        amount1_desired,
                    ...
                    message += (
                        (
                            ...
                            % (
                                token0_symbol,
                                token0_symbol,
                                amount0_desired / (10**token0_decimals),
                            )
                        )
                        .rstrip("0")
                        .rstrip(".")
