...
from txn_uniswapv3_helpers import COMP, RETH2, SWISE, SETH2, LDO, bcolors, get_best_rate, swap_selected_token, json_file_download, restart_end
    ...
    for reward_token in REWARDS_TOKENS:
        ...
        if int(swap_token_option) != j:
            ...
            while True:
                try:
                    if amount == 'max':
                        amount = token_balance / (10**reward_token_decimals)
                    else:
                        amount = float(amount)
                        if amount > token_balance / (10**reward_token_decimals):
                            print()
